...
int ComputeTsBox(float redshift, float prev_redshift, struct UserParams *user_params, struct CosmoParams *cosmo_params,
                  ...
                  struct EnergyInjectedPBH*energy_injected_PBH, 
...
//                        Throw(ParameterError);
        ...
        if(flag_options->USE_MASS_DEPENDENT_ZETA) {
            ...
            #pragma omp parallel shared(del_fcoll_Rct,dxheat_dt_box,dxion_source_dt_box,dxlya_dt_box,dstarlya_dt_box,previous_spin_temp,\
                    ...
                    private(box_ct,xHII_call, J_PBH_dt, dxion_PBH_dt, M_PBH, EFF_PBH, L_Edd, M_Edd, m_dot,L_PBH, n_ion_PBH      ) num_threads(user_params->N_THREADS)
            { 
                #pragma omp for
                for (box_ct=0; box_ct<HII_TOT_NUM_PIXELS; box_ct++){
            ...
            for (R_ct=global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct--;){
                ...
                #pragma omp parallel shared(dxheat_dt_box,dxion_source_dt_box,dxlya_dt_box,dstarlya_dt_box,dfcoll_dz_val,del_fcoll_Rct,freq_int_heat_tbl_diff,\
                    ...
                    private(box_ct,x_e,T,dxion_sink_dt,dxe_dzp,dadia_dzp,dspec_dzp,dcomp_dzp,dxheat_dzp,J_alpha_tot,T_inv,T_inv_sq, J_PBH_dt, dxion_PBH_dt, M_PBH, EFF_PBH, L_Edd, M_Edd, m_dot,L_PBH, n_ion_PBH \
...
// .........................................................................................................................
                            // definimos las cuentas para los PBH
                            
                            // Luminosidad de Eddington 
                            L_Edd = 1.3e38 * astro_params->M_PBH // la masa en Msolares y las unidades en erg/s (1)
                                
                            M_Edd = L_Edd/ ( astro_params->EFF_PBH *pow(C / 100000.0, 2))  // Masa de Edd para PBH  (2)
                            
                            //la m_dot es: Relación adimensional entre la tasa de acreción y la tasa de Eddington
                            m_dot = (energy_injected_PBH->M_ACR_PBH) / M_Edd    //                              (3)
                            // Luminosidad del PBH      
                            L_PBH = L_Edd * m_dot*pow(m_dot/(energy_injected_PBH->m_crit),energy_injected_PBH->BETA_PBH )
                            
                            // Tasa de ionizacion generada por los PBH
                                
                            n_ion_PBH = L_PBH / (hplank*3.3e15)
                           
                            // First let's do dxe_dzp //. asi esta en el 21cmFAST
                            dxion_sink_dt = alpha_A(T) * global_params.CLUMPING_FACTOR * x_e*x_e * f_H * prefactor_1 * \
                                            (1.+curr_delNL0*growth_factor_zp);
                           
                            // le sumamos el efecto del PBH a los MINI HALOS
                            dxion_PBH_dt = Constants.h->C_PBH *n_ion_PBH
                            
                            if (flag_options->USE_MINI_HALOS){
                                dxe_dzp = dt_dzp*(dxion_source_dt_box[box_ct] + dxion_source_dt_box_MINI[box_ct] + dxion_PBH_dt - dxion_sink_dt );// .......................................................................................................................
                            }
                            else{
...
// definimos el J_PBH_dt ............................................................................................
                           ...
                           // L_Edd = 1.3e38 * astro_params->M_PBH // la masa en Msolares y las unidades en erg/s (1)
                            ...
                            // Luminosidad del PBH      
                            //L_PBH = L_Edd * m_dot*pow(m_dot/(energy_injected_PBH->m_crit),energy_injected_PBH->BETA_PBH )
                            // Flujo de energia injectada en el IGM por el PBH
                            J_PBH_dt = energy_injected_PBH->n_PBH *(L_PBH)  //                     (4)                   
                            
                            // FLujo total y modificamos solo para MINI HALOS    
                            J_alpha_tot = ( dxlya_dt_box[box_ct] + dstarlya_dt_box[box_ct] ); //flux
                            if (flag_options->USE_MINI_HALOS){
                                J_alpha_tot_MINI = ( dxlya_dt_box_MINI[box_ct] + dstarlya_dt_box_MINI[box_ct] + J_PBH_dt ); //not really d/dz, but the lya flux......................................................................................
