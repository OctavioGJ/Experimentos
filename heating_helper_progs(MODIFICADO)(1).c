//user_params_hf, cosmo_params_hf, astro_params_hf y flag_options_hf son punteros a estructuras de tipos UserParams, CosmoParams, AstroParams y FlagOptions, 
//
...
// determine_zpp_min y zpp_bin_width son variables de tipo float, que generalmente se utilizan para almacenar números en punto flotante con menor precisión. Las variables de tipo double (más precisión) como BinWidth_pH, inv_BinWidth_pH
...
// Esta función es para calcular una masa mínima (MMIN) de iones en función de un parámetro de z
// Condicional para Estado del IGM

//  las variables que usan estan en 21cmFAST.h
...
// -----------------------------------------------------------------------------------------------------------------------
// Estos son todos parametros colicionales
// Ecuaciones del cosmology at low frequencies:  Furnaletto tabla 3,   pag 29
...
// ----------------------------------------------------------------------------------------------------------
//  Usa Ec 20 pag 24,  del cosmology at low frequencies:  Furnaletto et al 2008
...
// función estima la derivada de la fracción colapsada de estructuras....
// Se basa en el cálculo de la fracción colapsada en dos puntos cercanos al valor de z
// La ecuacion es la derivada de la fraccion colapsada...
...
float dfcoll_dz(float z, float sigma_min, float del_bias, float sig_bias)
{
    ...
    dz = 0.001;
    // z1 y z2 son puntos de redshift que están ligeramente por encima y por debajo del valor de z.
    z1 = z + dz;
    z2 = z - dz;
...
//la función init_FcollTable, inicializa una tabla de fracciones de colapso (Fcoll o FgtrM) en función del redshift.
...
int init_FcollTable(float zmin, float zmax)
{
    ...
    //Cálculo de la Fracción de Colapso
...
// función T_RECFAST, que se utiliza para interpolar temperaturas a partir de datos leídos de un archivo, específicamente para // el modelo RECFAST, este método se usa para calcular la evolución de la temperatura del gas en la historia del universo.

//  TRABAJA ASI: 1 - Leer y almacenar datos de temperatura desde un archivo.
//               2 - Configurar la interpolación para acceder a estos datos de manera eficiente.
//               3 - Evaluar la temperatura para un redshift dado mediante la interpolación.
...
double T_RECFAST(float z, int flag)
{
    ...
    char filename[500];
    // Leer datos desde un archivo y preparar la interpolación (flag = 1).
    if (flag == 1) {
    ...
    // Liberar la memoria ocupada por la interpolación (flag = 2).
    if (flag == 2) {
        // Free memory
...
// -----------------------------------------------------------------------------------------------------------------------
// Ionization fraction from RECFAST. //
// se encarga de calcular la fracción de ionización del hidrógeno en función del redshift.
...
//  el paper :  https://www.annualreviews.org/content/journals/10.1146/annurev.astro.39.1.19
...
double spectral_emissivity(double nu_norm, int flag, int Population)
{
    ...
    switch(flag){
        case 2:
            ...
            // Flag = 2: Calcular para el caso LW (Lyman-Werner).
...
// ---------------------------------------------------------------------------------------------------------------------
// La función xcoll calcula la tasa de colisión para un determinado z, temperatura Tk, densidad de materia perturbada  y fracción de electrones libres xe.
//  ver  Furnaletto  cosmology at low frequencies :  pag 29

// xcoll =  Tasa de colisión de hidrógeno + Tasa de colisión de electrones + Tasa de colisión de protones
...
//  ver  Furnaletto  cosmology at low frequencies :  pag 28 , Ec 36
double kappa_10(double TK, int flag)
{
...
// ******************************************************************** //
//  Esta función calcula Jalpha con la tasa de absorción de fotones, ajustado por el parámetro de ionización Jα y la Prof óptica de Gunn-Peterson (GP).

// tgp: calcula la profundidad óptica de Gunn-Peterson usando la función taugp.
// Stilde: calcula el factor de corrección usando Salpha_tilde, que depende de las temperaturas y la profundidad óptica.
// x: Finalmente, se calcula un valor proporcional a la densidad y ajustado por Jα y el redshift z.
...
// Contribución del Hidrógeno Ionizado (HI)
// Contribución del Helio Ionizado (HeI)
// Contribución del Helio Doble Ionizado (HeII)
// La función integra las contribuciones de diferentes especies (hidrógeno y helio) al calentamiento del medio intergaláctico.
...
// La función calcula un integral relacionada con la ionización de diferentes especies (hidrógeno y helio) en función de la frecuencia νν
// Ionización del Hidrógeno Neutro (HI) Para el hidrógeno, la contribución a la ionización
// Ionización del Helio Neutro (HeI)
// Ionización del Helio Doble Ionizado (HeII)
...
//  la función integrand_in_nu_lya_integral,   calcula el integrando para el proceso de absorción de fotones en la línea de Lyman-alpha (Lyα) 
//  creo que es la ecuacion 80  pag 55 de furnaletto....cosology....
// integqral para fotones Ly alpha
...
// ----------------------------------------------------------------------------------------------------------------
// Se utiliza el método de integración   gsl_integration_qag   para calcular la integral entre lower_int_limit y global_params.NU_X_MAX * NU_over_EV
// integra sobre todas las frecuencias nu
...
// IMPORTANTE !!!!   Esta función es para calcular la opacidad y los efectos de la ionización en el IGM.
...
double tauX_integrand_MINI(double zhat, void *params){
    ...
    //estas ecuaciones se utilizan para calcular cómo se comportan las propiedades de la materia y la radiación en un universo en expansión, considerando la densidad, la distancia de propagación y los efectos de z
...
// ----------------------------------------------------------------------------------------------------------------
// La función tauX_integrand calcula la densidad de ionización en función del desplazamiento al rojo, ajustando factores como la densidad de hidrógeno, el efecto del z en la frecuencia y el llenado de hidrógeno.
