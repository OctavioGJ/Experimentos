...
// -------------------------------------------------------------------------------------------------------------------------------
    ...
    float erfc_denom, 
        ...
        ION_EFF_FACTOR, 
        growth_factor_dz;
    float Splined_Fcoll_MINI, 
        prev_dens, 
        ION_EFF_FACTOR_MINI, 
        prev_Splined_Fcoll,
        prev_Splined_Fcoll_MINI;
    float ave_M_coll_cell, 
        ave_N_min_cell, 
        pixel_volume, 
        density_over_mean;
...
// usa N_THREADS, que se encuentra en  21cmFAST.h linea 29
    ...
    int *overdense_int_boundexceeded_threaded = calloc(user_params->N_THREADS,sizeof(int));
        ...
        // CRIT_DENS_TRANSITION  esta en Globals.h
    ...
    double ave_log10_Mturnover, 
           ave_log10_Mturnover_MINI;

    float Mlim_Fstar, Mlim_Fesc;
    float Mlim_Fstar_MINI, Mlim_Fesc_MINI;
// Estas variables son parte integral de los cálculos relacionados con la formación de estructuras en el universo,  para la evolución de galaxias y en  la ionización. Almacenan información sobre masas críticas y virialización, y utilizan tanto valores directos como logarítmicos para facilitar los cálculos.
...
//  Inicialización de Variables Relacionadas con la Temperatura
...
// Cálculo del Factor de Eficiencia de Ionización.
...
// -------------------------------------------------------------------------------------------------------------------------------
    ...
    // For recombinations
    if(flag_options->INHOMO_RECO) { // INHOMO_RECO esta en 21cmFAST.h, dentro de FlagOptions linea 81
...
// -------------------------------------------------------------------------------------------------------------------------------
// cálculo de ZSTEP según el desplazamiento al rojo (redshift)
        
     if (prev_redshift < 1) //deal with first redshift
		 ZSTEP = (1. + redshift) * (global_params.ZPRIME_STEP_FACTOR - 1.);
     else
        ZSTEP = prev_redshift - redshift;  // DEFINICION DE ZSTEP
...
// IMPORTANTE ... aparecen los MINI HALOS
...
// Este fragmento de código gestiona la asignación de memoria para múltiples arreglos que almacenan datos relacionados con las densidades de ionización y la tasa de formación estelar
            if (flag_options->USE_MINI_HALOS){
...
//Inicialización Paralela de z_re_box
    if (prev_redshift < 1){
...
#pragma omp for
        ...
        if (flag_options->INHOMO_RECO) //Condicional para Recomendaciones de Recobros Inhomogéneos
            previous_ionize_box->dNrec_box   = (float *) calloc(HII_TOT_NUM_PIXELS, sizeof(float));
    }
    //set the minimum source mass
        // USE_MASS_DEPENDENT_ZETA en 21cmFAST.h  linea 78
    if (flag_options->USE_MASS_DEPENDENT_ZETA) {
        if (flag_options->USE_MINI_HALOS){
...
// Asignación de Memoria para densidades de Colapso
...
LOG_SUPER_DEBUG("previous density field calculated");
            ...
            //Asignación de Memoria para Arrays FFT
...
// reionization_feedback esta en UsefulFunctions.c  linea 998
...
// Definición: 𝑀crit, LWM crit, LW  es la masa mínima de un halo que puede fragmentarse y formar estrellas a través de la radiación de Lyman-Werner (LW). Esta radiación es capaz de dissociar moléculas de hidrógeno, afectando la temperatura y la dinámica del gas en el halo.
...
// parametros astrofisicos...
            ...
            Mturnover                 = pow(10., box->log10_Mturnover_ave);  // son las definiciones de Mturnover
            Mturnover_MINI            = pow(10., box->log10_Mturnover_MINI_ave); // def de Mturnover_MINI
            M_MIN           = global_params.M_MIN_INTEGRAL;  // def de M_MIN
...
// ------------------------------------------------------------------------------------------------------------------------            
    ...
    if (flag_options->USE_MASS_DEPENDENT_ZETA) {
        if (flag_options->USE_MINI_HALOS){
            // ESTE ES PARA ACG
            // ION_EFF_FACTOR viene de 
            //  mean_f_coll viene de   " previous_ionize_box->mean_f_coll = 0.0" , linea anterior 380
...
// CAlculos de los respectivos log10_Mturnover_MINI , log10_Mturnover
    if(MINI_HALOS) {
