...
// input usados en la ecuacioin de rayos X
double *dxheat_dt_box, *dxion_source_dt_box, *dxlya_dt_box, *dstarlya_dt_box; // para ACG
double *dxheat_dt_box_MINI, *dxion_source_dt_box_MINI, *dxlya_dt_box_MINI, *dstarlya_dt_box_MINI; // para MCG
...
//Variables needed for heating calculations  (estan en heating_helper_progs.c)
...
if (LOG_LEVEL >= DEBUG_LEVEL){
    ...
    bool initialization_required = fabs(initialised_redshift - perturbed_field_redshift) > 0.0001;
        ...
        // para la eficiencia de ionizacion ver la ecuacion 83 (pag 56) de "Cosmology at low frequencies: ..." Furnaletto 2006
        // aca define lo que es ION_EFF_FACTOR y ION_EFF_FACTOR_MINI
    ...
    // Initialise arrays to be used for the Ts.c computation // MANEJO DE MEMORIA
...
// Definicion de las masas limites que se van a usar:
            // usa la funcion  float Mass_limit_bisection(float Mmin, float Mmax, float PL, float FRAC) linea 100 de ps.c
...
// aca aparece por primera ves L_X
        
        // para "L_X" ver el paper de Greig Ec 6, el paper es: 21CMMC with a 3D light-cone: the impact of the co-evolution
        // approximation on the astrophysics of reionization and cosmic dawn. MNRAS 477, 3217–3229 (2018)
...
// --------------------------------------------------------------------------------------------------------------------
                            ...
                            // dt_dzp : linea 1515 del original
                                                    
                            
                     
// LInea del archivo original  -  en donde aparece   dxheat_dt_box
//                   18    double *dxheat_dt_box, *dxion_source_dt_box, *dxlya_dt_box, *dstarlya_dt_box;
//                   19    double *dxheat_dt_box_MINI, *dxion_source_dt_box_MINI, *dxlya_dt_box_MINI, *dstarlya_dt_box_MINI;
//                   20    double *dstarlyLW_dt_box, *dstarlyLW_dt_box_MINI;
//                   272   dstarlyLW_dt_box = (double *) calloc(HII_TOT_NUM_PIXELS,sizeof(double));
//                   273   dxheat_dt_box_MINI = (double *) calloc(HII_TOT_NUM_PIXELS,sizeof(double));
//                   274   dxion_source_dt_box_MINI = (double *) calloc(HII_TOT_NUM_PIXELS,sizeof(double));
                    
 // ahora  como  en linea 273   dxheat_dt_box_MINI = (double *) calloc(HII_TOT_NUM_PIXELS,sizeof(double)); 
                            // entonces:
                            // #define HII_TOT_NUM_PIXELS (unsigned long long)(HII_D*HII_D*HII_D_PARA) esto esta en "indexing.c" linea 64      
                            
                            
//   dxheat_dt_box[box_ct]:   debe ser la tasa de calentamiento por rayos X por unidad de tiempo para el "box" o celda espacial correspondiente (indicado por box_ct). Esta tasa puede depender de varios factores, como la luminosidad de las fuentes de rayos X y su distribución en el espacio.
                            
//dt_dzp:  Es el cambio en el desplazamiento al rojo, que está relacionado con la evolución del tiempo en el contexto del modelo.
                            
// k_B: Es la constante de Boltzmann, que se usa para relacionar la energía térmica con la temperatura.
//x_e: La fracción de electrones libres en el medio, que afecta cómo los rayos X interactúan con el gas.                           
...
// --------------------------------------------------------------------------------------------------------------------
                    ...
                    num_threads(user_params->N_THREADS)
            {
                ...
                for (box_ct=0; box_ct<HII_TOT_NUM_PIXELS; box_ct++){
                    ...
                    if (!NO_LIGHT){
// IMPORTANTE           // Now determine all the differentials for the heating/ionisation rate equations
                        for (R_ct=global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct--;){
...
// ACA ESTA!!!!     // next, X-ray heating
                    dxheat_dzp = dxheat_dt * dt_dzp * 2.0 / 3.0 / k_B / (1.0+x_e);

                    // DONDE :   dxheat_dt_box = (double *) calloc(HII_TOT_NUM_PIXELS,sizeof(double));
                    // dxheat_dt  ver linea 2335 del original
                    // dt_dzp : linea 1515 del original
 ...
 // --------------------------------------------------------------------------------------------------------------------
                    ...
                    ////Function  double Energy_Lya_heating(double Tk, double Ts, double tau_gp, int flag)  en heating_helper_progs linea 1339
{                             
                    if (flag_options->USE_LYA_HEATING) {
                        E_continuum = Energy_Lya_heating(T, previous_spin_temp->Ts_box[box_ct], taugp(zp,curr_delNL0*growth_factor_zp,x_e), 2);
                        E_injected = Energy_Lya_heating(T, previous_spin_temp->Ts_box[box_ct], taugp(zp,curr_delNL0*growth_factor_zp,x_e), 3);
