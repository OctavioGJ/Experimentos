...
struct AstroParams{
    ...
    // para 21cm-BHFAST
    float M_PBH;    // en Masa Solares 
    float EFF_PBH; // valores entre (0,1)
...
// para 21cm-BHFAST
struct EnergyInjectedPBH{
    float n_PBH     // = 0.1 
    float BETA_PBH  // entre (0,1) 
    float m_crit    // = 0.01  paper 1803.09697v3
    float M_ACR_PBH // hay que definir 
 };   
...
int ComputeTsBox(float redshift, float prev_redshift, struct UserParams *user_params, struct CosmoParams *cosmo_params,
                  ...
                  struct TsBox *this_spin_temp, struct EnergyInjectedPBH*energy_injected_PBH);
